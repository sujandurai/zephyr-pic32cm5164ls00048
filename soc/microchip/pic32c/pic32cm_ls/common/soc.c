...
#define TIMEOUT_CYCLES 500000U

/**
 * @brief Configure system clocks for 48 MHz operation via FDPLL96M.
 */
void pic32cm_ls_clock_init(void)
{
	/* 1. Flash wait states: 2 wait states for 48 MHz at PL2 */
	...
	/* 3. Configure and enable OSC16M at 16 MHz */
	...
	/* 9. Wait for FDPLL Lock */
	for (volatile uint32_t t = 0; t < TIMEOUT_CYCLES; t++) {
		if (OSCCTRL_REGS->OSCCTRL_DPLLSTATUS & OSCCTRL_DPLLSTATUS_LOCK_Msk) {
			break;
		}
	}

	/* 10. Switch main GCLK0 to FDPLL96M (48 MHz) */
